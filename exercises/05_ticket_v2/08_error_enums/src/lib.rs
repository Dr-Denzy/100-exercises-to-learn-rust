...
impl TicketNewError {
    fn message(&self) -> &str {
        match self {
            TicketNewError::TitleError => "Title cannot be empty. And Title cannot be longer than 50 bytes",
            TicketNewError::DescriptionError => "Description cannot be empty. And Description cannot be longer than 500 bytes"
        }
    }
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::TitleError) => panic!("Title cannot be empty. And Title cannot be longer than 50 bytes"),
